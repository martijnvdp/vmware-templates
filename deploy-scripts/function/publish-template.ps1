...
function publish-template {
    ...
    if ($Template_os -eq "ubuntu") {
        packer build -on-error=ask -force --var-file $Template_var_file -var "vcenter_username=$($Credential.username)"  -var "vcenter_password=$($Credential.GetNetworkCredential().Password)"  -var "ssh-password=$winadmin_password" $Template_file   
    }
}